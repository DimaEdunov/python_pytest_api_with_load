...
from src.flow_constructors.domains import get_park_full_name, get_angela_login_credentials, get_uuid_status
...
def test_media_lb_nj_nv(i, driver, application_parameters):
    print_log("Current run in loading test", f"\nCurrent run is: {i + 1}")
    ###### Explanation ######
    # Domain = LB (LEGOLAND® Billund)
    # Attraction park is nj = Ninjago - The Ride
    # nv = Ninjago - The Ride Digital
    # The result in this test needs to be: 1 dynamic overlay
    ###### End of Explanation ######

    try:
        function_name = test_media_lb_nj_nv.__name__  # Get the name of the function
        substrings = function_name.split("_")[2:]  # Select substrings starting from the third one
        lb, nj, nv = substrings  # Unpack the substrings into separate variables
        ...
        if application_parameters['environment'] not in ["us", "uk", "ap"]:
            ...
            uuid_list = connector_helper_object.get_origin_uuids(
                Connector.get_connector_main_path() + AttractionParameters.get_folder_path('LB-NV'))
        ...
        connector_helper_object.photos_drag_and_drop_and_upload(
            attraction_names=AttractionParameters.LB_NV_ATTRACTION_NAMES.value)
        ...
        if application_parameters['environment'] not in ["us", "uk", "ap"]:
            ...
            email = get_angela_login_credentials('user')
            user_id = api_helper_media.api_get_userid_by_email_request(application_parameters['environment'], email,
                                                                       domain=lb)
            ...
            allure.attach(driver.get_screenshot_as_png(), name="test_media_lb_nj_nv",
...
@pytest.mark.run(order=1)
@pytest.mark.parametrize("i", range(1))
def test_media_lb_td_dv(i, driver, application_parameters):
    print_log("Current run in loading test", f"\nCurrent run is: {i + 1}")
    ###### Explanation ######
    # Domain = LB (LEGOLAND® Billund)
    # Attraction park is td = The Dragon
    # dv = The Dragon Digital
    # The result in this test needs to be: 1 dynamic overlay
    ###### End of Explanation ######

    try:
        function_name = test_media_lb_td_dv.__name__  # Get the name of the function
        substrings = function_name.split("_")[2:]  # Select substrings starting from the third one
        lb, td, dv = substrings  # Unpack the substrings into separate variables
        ...
        if application_parameters['environment'] not in ["us", "uk", "ap"]:
            ...
            uuid_list = connector_helper_object.get_origin_uuids(
                Connector.get_connector_main_path() + AttractionParameters.get_folder_path('LB-DV'))
            for uuid in uuid_list:
                api_helper_media.api_get_media_id_by_uuid(application_parameters['environment'], uuid=uuid,
                                                          list_of_media_ids=media_ids_list_for_deletion)
        ...
        connector_helper_object.photos_drag_and_drop_and_upload(
            attraction_names=AttractionParameters.LB_DV_ATTRACTION_NAMES.value)
        ...
        if application_parameters['environment'] not in ["us", "uk", "ap"]:
            ...
            allure.attach(driver.get_screenshot_as_png(), name="test_media_lb_td_dv",
                          attachment_type=AttachmentType.PNG)
